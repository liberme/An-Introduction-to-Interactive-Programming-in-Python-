...
def mouseclick(pos):
    ...
    if state == 0: #first game
        for number in range(len(card_list)):
            if pos[0] > card_edge and pos[0] < card_edge + 50 and exposed[number] == False: # position and expose detected
                exposed[number] = True 
                counter += 1
                card_check1 = number
            card_edge += 50 # change position
        state = 1
    elif state == 1:
        for number in range(len(card_list)):
            if pos[0] > card_edge and pos[0] < card_edge + 50 and exposed[number] == False: # position and expose detected
        ...
        if card_list[card_check1] == card_list[card_check2]: # if pair set True
            ...
            # when two unpair card exposed , make sure player click other unexpose card , prevent only flip one card
            for number in range(len(card_list)):
                if pos[0] > card_edge and pos[0] < card_edge + 50 and exposed[number] == False: # position and expose detected
    ...
    return label.set_text(Turns) 

# cards are logically 50x100 pixels in size    
def draw(canvas):
    start_point = 10
    pos = 0 # for moving position
